...
namespace :seed do
  ...
  task dsls: [:environment, :games, :custom_levels] do
    DSLDefined.transaction do
      puts 'start'; start = Time.now.to_f
      i18n_strings = {}
      ...
      puts "Filenames: #{Time.now.to_f - start}"; start = Time.now.to_f
      dsl_data_i18n = dsls.map do |filename, dsl_class|
        data, i18n = dsl_class.parse_file(filename)
        [dsl_class, data, i18n]
      end
      puts "dsl_data: #{Time.now.to_f - start}"; start = Time.now.to_f
      DSLDefined.parse_times.sort_by{|filename, time|time}.map{|x| puts x}
      dsl_data = dsl_data_i18n.map do |dsl_class, data, i18n|
        i18n_strings.deep_merge! i18n
        [dsl_class, data]
      end
      puts "dsl_data i18n: #{Time.now.to_f - start}"; start = Time.now.to_f
      levels = Level.all.index_by(&:name)
      puts "levels: #{Time.now.to_f - start}"; start = Time.now.to_f
      dsl_objects = dsl_data.map do |dsl_class, data|
        dsl_class.setup(data, levels)
      end.select(&:changed?)
      puts "dsl_objects: #{Time.now.to_f - start}"; start = Time.now.to_f
      DSLDefined.import(dsl_objects, validate: false)
      puts "Imported: #{Time.now.to_f - start}"; start = Time.now.to_f
